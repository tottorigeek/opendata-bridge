...
| **外部データソース連携** | 外部の REST API(JSON)や CSV の URL をデータセットに関連付け、手動 / 定期(毎日)で取り込み。取得結果は CSV アップロードと同じ実体に書き込まれるため、カタログ・マージ・公開 API がそのまま使える。 |
| 承認フロー | `下書き → 承認待ち → 公開 / 差し戻し`。公開は組織の ADMIN のみ承認可能。 |
| 公開カタログ | 公開データセットをキーワード・組織種別・タグ・**自治体(都道府県 / 市区町村)** で横断検索。CSV ダウンロード対応。 |
| 地域メタデータ | データセットに「対象地域」、組織に「所在地」を設定。対象地域が未設定のデータセットは組織の所在地で絞り込まれる。市区町村は総務省の全国地方公共団体コード(47 都道府県 / 1,741 市区町村)に基づくマスタから選択。 |
...
別システムのデータベースが公開している API を、データセットに **関連付けて随時引き込む**
仕組みです。CSV アップロードを置き換えるものではなく、**同じ器を埋める 2 つ目の経路**として
共存します。

```
CSV アップロード ─┐
                  ├─→ storage の datasets/{id}.csv ─→ カタログ / ダウンロード / マージ / REST API v1
外部 API 取り込み ─┘
```

取得したデータは既存の CSV ストレージ(`lib/storage.ts`)へ書き出されます。この一点に
揃えているため、**下流の機能は一切変更していません**。API 取り込みのデータセットも、
そのままマージエンジンの入力にでき、公開カタログにも載り、REST API v1 からも読めます。

| 項目 | 対応内容 |
| --- | --- |
| 取得方式 | REST API(JSON)/ CSV の URL |
| 認証 | なし / Bearer トークン / 任意のヘッダ / クエリパラメータ |
| レコード位置 | ドット区切りで指定(CKAN なら `result.records`)。直下が配列なら省略可 |
| 列マッピング | 取得元のキー → 保存する列名。未指定なら全キーを自動で列にする |
| 同期 | 手動(いつでも)/ 定期(毎日・`syncMode = SCHEDULED`) |
| 接続テスト | 保存前に取得を試し、列と先頭 10 行を確認できる |
| 履歴 | 成功 / 失敗・行数・所要時間・実行種別を `SyncRun` に記録 |

設定はデータセット詳細画面 `/dashboard/datasets/[id]` の「外部データソース」パネルから行います。

**定期同期の有効化**: `CRON_SECRET` を環境変数に設定してください(未設定なら定期同期は
503 で無効。手動同期は使えます)。スケジュールは [vercel.json](vercel.json) の
`0 18 * * *`(UTC)= **毎日 03:00 JST** です。

**取り込みの設計上の判断**:

- **失敗しても既存データを消しません。** 取得できなかったからといって公開中のデータを
  空にする方が害が大きいためです。失敗は履歴と画面に残ります。
- **取得先 URL は SSRF 対策を通します。** 利用者が入力した URL をサーバーが取得しに行くため、
  スキーム・ポートの制限に加えて、名前解決した全 IP が公開アドレスであることを確認し、
  リダイレクトも 1 ホップごとに再検証します(`lib/sources/fetch.ts`)。
  クラウドのメタデータ(`169.254.169.254`)や内部ホストへは到達できません。
- **認証情報は AES-256-GCM で暗号化して保存します**(`lib/crypto.ts`)。相手に提示する
  必要があるためハッシュ化はできませんが、平文では保存しません。画面にも再表示されません。
...
- `/dashboard` — ダッシュボード
  - `/dashboard/datasets` — 自組織のデータセット一覧 / `new`・`[id]`(外部データソース設定を含む)・`[id]/edit`
...
同梱データ(市区町村マスタ)の出典は [NOTICE](NOTICE) に記載しています。
